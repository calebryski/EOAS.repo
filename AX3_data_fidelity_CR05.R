...
cut2 <- axd[datetime>as.POSIXct("2021-07-24 20:00:00",tz = "UTC") & datetime<as.POSIXct("2021-09-17 08:00:00",tz = "UTC")]  
cut2
...
cut4 <- axd[datetime>as.POSIXct("2021-07-24 20:00:00",tz = "UTC") & datetime<as.POSIXct("2021-07-24 23:59:59",tz = "UTC")]  
cut4
...
for(i in 1:nrow(files)){
  ...
  # Subset data based on to_dat and eos_dat
  subset <- df[datetime>as.POSIXct(paste(ref$to_dat[i], "20:00:00"),tz = "UTC") & datetime<as.POSIXct(paste(ref$eos_dat[i], "08:00:00"),tz = "UTC")]
}
  # Expected
  expected <- seq(from = as.POSIXct(paste(ref$to_dat[1], "20:00:00")), to = as.POSIXct(paste(ref$eos_dat[1], "08:00:00")), 
                  by = "1 sec") ##### needs to be 0.08 sec but cant figure that one out, based on my reading of seq, the lowest unit of time that it can use  
  expected <- length(expected)*12.5 #### fix for the above line problem
...
file <- "AX3_copy/66620_SPH.csv"

df <- fread(file, header = FALSE, col.names = c("datetime", "X", "Y", "Z"))
subset <- df[datetime>as.POSIXct(paste("2021-05-25", "20:00:00"),tz = "UTC") 
             & datetime<as.POSIXct(paste("2021-07-12", "08:00:00"),tz = "UTC")]

x <- seq(from = as.POSIXct(paste("2021-05-25", "20:00:00")), 
         to = as.POSIXct(paste("2021-07-12", "08:00:00")), 
                by = "1 sec") 
x <- length(x)*12.5
